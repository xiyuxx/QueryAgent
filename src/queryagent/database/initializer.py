...
This module is the only Phase 1 code allowed to connect directly to
PostgreSQL. Query-time access is added later through the PostgreSQL MCP
server. The initializer is deterministic: resetting with the same seed
produces the same rows and embedding documents.
"""
...
def initialization_from_environment(dsn: str | None = None, **kwargs) -> InitializationResult:
    ...
    return initialize_production_database(
        resolved_dsn,
        embedding_model=os.environ.get("QUERYAGENT_EMBEDDING_MODEL", DEFAULT_EMBEDDING_MODEL),
        embedding_dim=int(os.environ.get("QUERYAGENT_EMBEDDING_DIM", str(DEFAULT_EMBEDDING_DIM))),
        **kwargs,
    )
